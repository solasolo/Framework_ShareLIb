...
namespace GLEO.MES.Network
{
    public class StreamBuffer
    {
        ...
        struct Cursor
        {
            public List<byte[]>.Enumerator it;
            public int pos;
        }
        ...
        private bool locate(int pos, ref Cursor cur)
        {
            ...
            {
                while (cur.it.MoveNext() && pos > 0)
                {
                    int n = cur.it.Current.Length;
                    if (pos >= n)
                    {
                        pos -= n;
                    }
        ...
        private bool ReadData(byte[] buffer, int pos, int len)
        {
            ...
            if (bret)
            {
                ...
                do
                {
                    ...
                    Array.Copy(cur.it.Current, cur.pos, buffer, c, n);
                    c += n;
                    cur.pos = 0;
                } 
                while (cur.it.MoveNext() && c < len);
        ...
        public byte ReadByte(int pos)
        {
            ...
            if (bret && cur.it.MoveNext())
            {
                data = cur.it.Current[cur.pos];
            }
